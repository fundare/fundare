...
class MyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
      ...
      final p2 = Offset(size.width/2.0, size.height-fromBottom); // bottom point
      ...
      canvas.drawCircle(Offset(p1.dx, size.height/2.0), 6, paint); // plot car altitude

      paint.color = Colors.blue;
      double halfAxisLength = (carAltitude-originalAltitude).abs()*1.5;
      double currAltiPosition = (carAltitude-currentAltitude)/halfAxisLength*size.height/2.0 + size.height/2.0;
      canvas.drawCircle(Offset(p1.dx, currAltiPosition), 4, paint); // plot user current altitude
  }

  @override
  bool shouldRepaint(MyPainter oldDelegate) {
      bool repaintBool = (oldDelegate.currentAltitude-currentAltitude).abs() > 0.5;
      return repaintBool;
  }
}